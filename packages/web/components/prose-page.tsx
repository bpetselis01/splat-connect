...
export function ProsePage({
  ...
  return (
    <article className="relative max-w-prose">
      ...
      <div className="mt-8 flex flex-col gap-6 text-sm leading-relaxed text-muted [&_h2]:text-lg [&_h2]:font-bold [&_h2]:text-ink [&_h3]:font-bold [&_h3]:text-ink [&_li]:mt-1.5 [&_p]:mt-3 [&_strong]:text-ink [&_ul]:list-disc [&_ul]:pl-5">
        {children}
      </div>
    </article>
  )
...
export function PullQuote({ children }: { children: React.ReactNode }) {
  return (
    <p className="tilt-2 my-2 rounded-2xl bg-mint-soft px-5 py-4 text-base font-bold leading-relaxed text-mint-deep">
      {children}
    </p>
  )
}
