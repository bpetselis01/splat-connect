...
- **`Button` needs no new `testID`; `TextField` does.** `Button` sets `accessibilityLabel={label}` on the pressable itself (`ui/Button.tsx:45`), which Android exposes as `content-desc` and Maestro matches unambiguously. `TextField` instead renders its `label` as a sibling `<Text>` carrying the same string as the `TextInput`'s own `accessibilityLabel`, so a text/label selector matches two nodes with an undetermined winner. The two sign-in inputs (`profile-screen.tsx`) carry explicit `testID="email-input"`/`testID="password-input"`, and the flows target them by `id` instead of `text`.
